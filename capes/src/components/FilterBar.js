...
function FilterBar({worksData, applyFilters, searchPerformed}) {
    ...
    const handleFilters = async () => {
        ...
        const editorsCount = {};
        worksData.forEach((work) => {
            const editor = work.authorships[0]?.institutions[0]?.display_name || "unknown";
        ...
        const publicationYears = worksData.map((work) => work.publication_year).filter(Boolean);
    ...
    const applySelectedFilters = () => {
        ...
        if (selectedTypes.length > 0) {
            filteredWorks = filteredWorks.filter(work => selectedTypes.includes(work.type));
        }

        // Filtros de áreas
        if (selectedAreas.length > 0) {
            filteredWorks = filteredWorks.filter(work => selectedAreas.includes(work.concepts[0]?.display_name));
        }
